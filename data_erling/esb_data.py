...
data  = pd.read_csv('/Users/erlingstraybugge/Library/CloudStorage/OneDrive-NTNU/Studier/10. semester/Masteroppgave/Koding/master-thesis/data_erling/hourly_data_areas.csv')
price_columns_main = ['System Price', 'SE1', 'SE2', 'SE3', 'SE4', 'FI', 'DK1', 'DK2', 'Oslo', 'Kr.sand', 'Bergen', 'Molde', 'Tr.heim', 'Tromsø']
coeff_columns_main = ['Month', 'Weekday', 'Weekend']

def read_data(columns, path='/Users/erlingstraybugge/Library/CloudStorage/OneDrive-NTNU/Studier/10. semester/Masteroppgave/Koding/master-thesis/data_erling/hourly_data_areas.csv'):
...
def calc_coeff(data, coeff_col, price_col):
    ...
    for coeff in coeff_set:
        coeff_avg = data[price_col].loc[data[coeff_col==coeff]].mean()
    ...
    for area in node_temp_list:
        for coeff in coeff_columns:
            ...
            for coefficient_value_key in coeffs_for_area.keys():    #Coeff_value_key is eg. hr=2 or weekday=4
                node_temp_2 = Node(value=coefficient_value_key)
...
if __name__=='__main__':
    data = read_data
    root_node = make_coeff_tree(data, ['System Price'], ['Hour', 'Weekday'])
